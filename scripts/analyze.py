...
def load_lessons() -> dict:
    p = DATA_DIR / "ai_lessons.json"
    if p.exists():
        with open(p) as f:
            return json.load(f)
    return {"lessons": [], "patterns": {"strengths": [], "weaknesses": [], "summary": ""}, "stats": {}}

def save_lessons(data: dict):
    with open(DATA_DIR / "ai_lessons.json", "w") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
...
def ai_make_trading_decision(ai_portfolio: dict, holdings_analysis: list, watchlist: list) -> list:
    ...
    # ── โหลดบทเรียนที่ผ่านมา ──
    lessons_data  = load_lessons()
    recent_lessons = lessons_data.get("lessons", [])[-10:]
    patterns      = lessons_data.get("patterns", {})
    stats         = lessons_data.get("stats", {})

    lessons_block = ""
    if recent_lessons:
        outcome_emoji = {"correct":"✅","incorrect":"❌","too_early":"⏰","pending":"⏳"}
        lesson_lines = "\n".join([
            f"  {outcome_emoji.get(l.get('outcome',''),'❓')} {l.get('action','')} {l.get('ticker','')} "
            f"({l.get('change_pct',0):+.1f}%): {l.get('lesson','')}"
            for l in recent_lessons
        ])
        strengths = ", ".join(patterns.get("strengths", [])) or "ยังไม่ทราบ"
        weaknesses = ", ".join(patterns.get("weaknesses", [])) or "ยังไม่ทราบ"
        acc = (stats.get("correct",0) / max(stats.get("total_decisions",1),1) * 100)
        lessons_block = f"""
บทเรียนจากการเทรดที่ผ่านมา (ใช้ประกอบการตัดสินใจ):
{lesson_lines}

Pattern ของตัวเอง:
- จุดแข็ง: {strengths}
- จุดอ่อน: {weaknesses}
- ความแม่นยำ: {acc:.0f}% ({stats.get('correct',0)}/{stats.get('total_decisions',0)} ครั้ง)
"""
...
- ใช้บทเรียนที่ผ่านมาเพื่อหลีกเลี่ยงข้อผิดพลาดเดิม
...
- มีเหตุผล fundamental ชัดเจน + มีเงินสดพอ
...
[
  {{
    "action": "BUY" | "SELL",
    "ticker": "TICKER",
    "reason_th": "เหตุผล fundamental 2-3 ประโยค + อ้างอิงบทเรียนถ้าเกี่ยวข้อง",
    "conviction": "high" | "medium",
    "size_pct": 5
  }}
]

ถ้าไม่มีโอกาสที่ดีพอ → ส่ง [] เท่านั้น
...
# ── Learning System ──────────────────────────────────────────────────────────
def ai_evaluate_decision(decision: dict, current_price: float,
                          news: list, thesis: str, still_holding: bool) -> dict | None:
    """AI ประเมินว่าการตัดสินใจที่ผ่านมาถูกหรือผิด และเพราะอะไร"""
    ticker      = decision["ticker"]
    action      = decision["action"]
    ...
    news_text    = "\n".join([f"- {n['title']}" for n in news[:5]]) or "ไม่มีข่าว"
    holding_str  = "ยังถืออยู่" if still_holding else "ขายแล้ว / ไม่ได้ถืออีก"
    action_th    = "ซื้อ" if action == "BUY" else "ขาย"

    prompt = f"""{THAI}

คุณเป็น AI fund manager กำลังทบทวนการตัดสินใจที่ผ่านมาอย่างซื่อสัตย์

การตัดสินใจ:
- วันที่: {entry_date}
- action: {action_th} {ticker}
- ราคาที่เข้า: ${entry_price:.2f}
- เหตุผลตอนนั้น: {reason}

สถานการณ์ปัจจุบัน:
- ราคาปัจจุบัน: ${current_price:.2f} ({change_pct:+.1f}% จากราคาที่เข้า)
- สถานะ: {holding_str}
...
Thesis ปัจจุบัน (ย่อ):
{thesis[:500] if thesis else "ไม่มี"}

ประเมินอย่างตรงไปตรงมา ตอบ JSON (ไม่มี ```):
{{
  "outcome": "correct" | "incorrect" | "too_early" | "pending",
  "outcome_detail": "อธิบาย 1-2 ประโยคภาษาไทย ว่าถูก/ผิดอย่างไร",
  "lesson": "บทเรียนที่ควรจำสำหรับการเทรดครั้งต่อไป (ภาษาไทย ชัดเจน)",
  "mistake": "ข้อผิดพลาดที่เกิดขึ้นถ้ามี หรือ null",
  "what_worked": "สิ่งที่วิเคราะห์ได้ถูกต้องถ้ามี หรือ null"
}}

pending = ยังเร็วเกินไปที่จะตัดสิน
too_early = ทิศทางถูกแต่จังหวะเร็วหรือช้าเกินไป
"""
    text = call_ai(prompt).strip()
    try:
        if "```" in text:
        ...
        result.update({
            "decision_id":     f"{entry_date}_{ticker}_{action}",
            "date_reviewed":   bangkok_now().strftime("%Y-%m-%d"),
            "ticker":          ticker,
            "action":          action,
...
def ai_extract_patterns(lessons: list) -> dict:
    """AI สรุป pattern จุดแข็ง/จุดอ่อนจากบทเรียนทั้งหมด"""
    if len(lessons) < 2:
        return {"strengths": [], "weaknesses": [],
                "summary": "ยังมีบทเรียนน้อยเกินไป — รอเพิ่มเติม"}

    lessons_text = "\n".join([
        f"- {l.get('date_reviewed','')}: {l.get('action','')} {l.get('ticker','')} "
        f"→ {l.get('outcome','')} | บทเรียน: {l.get('lesson','')}"
        for l in lessons[-20:]
    ])

    prompt = f"""{THAI}

วิเคราะห์รูปแบบจากบทเรียนการเทรดที่สะสมมา:

{lessons_text}

ตอบ JSON (ไม่มี ```):
{{
  "strengths": ["จุดแข็ง 1 ภาษาไทย", "จุดแข็ง 2"],
  "weaknesses": ["จุดอ่อน 1 ภาษาไทย", "จุดอ่อน 2"],
  "summary": "สรุปภาพรวมสไตล์การเทรด 2 ประโยค"
}}
"""
    text = call_ai(prompt).strip()
    try:
        ...
        return {"strengths": [], "weaknesses": [], "summary": "วิเคราะห์ pattern ไม่ได้"}


def review_and_learn(ai_portfolio: dict) -> list:
    """ทบทวนการเทรดที่ผ่านมา สกัดบทเรียน อัพเดท pattern"""
    decisions = ai_portfolio.get("decisions", [])
    if not decisions:
        return []

    lessons_data   = load_lessons()
    reviewed_ids   = {l.get("decision_id") for l in lessons_data.get("lessons", [])}

    # เลือกการเทรดที่ยังไม่ได้ review และมีข้อมูลพอ
    to_review = [
        d for d in decisions
        if d.get("status") == "executed"
        and f"{d.get('date','')}_{d.get('ticker','')}_{d.get('action','')}" not in reviewed_ids
    ]

    if not to_review:
        print("  ไม่มีการเทรดใหม่ที่ต้อง review")
        return []

    new_lessons = []
    print(f"  Review {len(to_review)} การเทรด...")
    ...
    for d in to_review:
        ...
        still_holding = any(h["ticker"] == ticker for h in ai_portfolio.get("holdings", []))

        lesson = ai_evaluate_decision(d, current_price, news, thesis, still_holding)
        if lesson:
            new_lessons.append(lesson)
            print(f"    {ticker}: {lesson.get('outcome','?')}")
        time.sleep(2)

    if new_lessons:
        lessons_data.setdefault("lessons", []).extend(new_lessons)

        # อัพเดท stats
        stats = lessons_data.setdefault("stats", {"total_decisions": 0, "correct": 0, "incorrect": 0, "pending": 0})
        for l in new_lessons:
            stats["total_decisions"] = stats.get("total_decisions", 0) + 1
            outcome = l.get("outcome", "pending")
            if outcome == "correct":
        ...
        # อัพเดท pattern ถ้ามีบทเรียนพอ
        all_lessons = lessons_data["lessons"]
        if len(all_lessons) >= 2:
            lessons_data["patterns"] = ai_extract_patterns(all_lessons)

        save_lessons(lessons_data)

        # ส่ง Telegram สรุปบทเรียน
        outcome_emoji = {"correct": "✅", "incorrect": "❌", "too_early": "⏰", "pending": "⏳"}
        lines = [
            f"{outcome_emoji.get(l['outcome'],'❓')} *{l['action']} {l['ticker']}* ({l['change_pct']:+.1f}%)\n"
            f"   {l.get('outcome_detail','')}\n"
            f"   📝 บทเรียน: _{l.get('lesson','')}_"
            for l in new_lessons
        ]
        stats = lessons_data.get("stats", {})
        acc = (stats.get("correct", 0) / max(stats.get("total_decisions", 1), 1) * 100)
        send_telegram(
            f"📚 *AI บทเรียนประจำสัปดาห์*\n\n"
            + "\n\n".join(lines)
            + f"\n\n📊 *สถิติรวม:* ถูก {stats.get('correct',0)}/{stats.get('total_decisions',0)} ครั้ง ({acc:.0f}%)"
        )
...
def _lessons_snapshot() -> dict:
    """Return a compact lessons summary for dashboard.json"""
    data = load_lessons()
    return {
        "recent":   data.get("lessons", [])[-5:],
...
def update_dashboard_data(portfolio: dict, holdings_analysis: list,
    ...
    dashboard = {
        ...
        "ai": {
            ...
            "pending_decisions": (ai_portfolio or {}).get("pending_decisions", []),
...
def run_weekly():
    ...
    print("  Reviewing past decisions...")
    review_and_learn(ai_portfolio)
    ai_portfolio = load_ai_portfolio()   # โหลดใหม่เพราะ review อาจแก้ไขข้อมูล
